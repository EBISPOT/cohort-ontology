...
def main():
    ...
    for r in read(EVIDENCE):
        ...
        if r["definition"].strip():
            rows[r["ID"]] = (r["definition"].strip(), r["source PMID"] or r["source URL"])
    for r in read(CURATED):
        if r["definition"].strip():
            rows[r["ID"]] = (r["definition"].strip(), r["source"])
        else:
            rows.pop(r["ID"], None)
    ...
    with open(TEMPLATE, "w", newline="", encoding="utf-8") as f:
        ...
        for cid in sorted(rows):
            definition, source = rows[cid]
            w.writerow([cid, "owl:NamedIndividual", labels.get(cid, ""), definition, source, DRAFTED_BY, CHECKED_BY])
