...
COHO's own checks include `subset-example`: a cohort in the EGA, GWAS, MetaboLights or PRIDE subset must have an example study (`IAO:0000112`) from that resource, an `EGAS`, `GCST`, `MTBLS` or `PXD` accession respectively. `catalogue-subset` does the same for the catalogue subsets: a term in the GEC subset must carry a cross-reference to its entry in the Global Exposome Cohort Catalogue (and a term with such a cross-reference must be in the subset), and a term in the IHCC subset must be a member of the International Health Cohorts Consortium (COHO:0002701), whose atlas the listing is.
