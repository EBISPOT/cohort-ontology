...
- A cohort that is part of another is linked with `isSubCohortOf`; a cohort that belongs to an aggregation is linked from the aggregation with `hasCohort`. Each such assertion carries a verbatim quote and its source URL.
...
- Every cohort has one example study (`IAO:0000112`): an EGA study, GWAS Catalog study or paper, with a quote naming the cohort. Quotes are copied verbatim from fetched text; PMIDs, accessions and quotes are never reconstructed from memory. If something cannot be verified, say so in the curation table rather than guessing.
...
- No "all known studies" cross-references: one example per cohort.

## Pipeline and build

- Curated evidence lives in `src/curation/*_curated.tsv` (which override the `*_evidence.tsv` files). The scripts in `src/scripts` turn them into `src/templates/*.tsv`; `om make recreate-components` builds the components. Do not hand-edit generated templates or components.
