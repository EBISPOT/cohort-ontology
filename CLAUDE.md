...
- **Placeholders are the one exception.** A catalog name that no curator has worked on can be minted as a placeholder: an individual of the `temporary unclassified` holding class with `IAO:0000114` (has curation status) `IAO:0000124` (uncurated), the catalog's name as its label, its ids and other names as synonyms (editor notes on shared acronyms still apply), and the PGS subset, but no definition, example, location or links. The definition, example and subset-example checks skip uncurated terms. Curating a placeholder means giving it everything above and removing its curation status.
